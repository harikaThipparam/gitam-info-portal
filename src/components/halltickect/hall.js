...
const FindHallTicket = () => {
  ...
  const ChangeHall = (event) => {
    const HTInput = parseInt(event.target.value);
    setHallticket(HTInput);
    if (HTInput !== 40404) {
      const FullDataObject = StudentsData.slice(0, 100).find(
        (each) => each.hallticketNumber === HTInput
      );

      setFullStudentData(FullDataObject);
    }
  };

  const StudentName =
    FullStudentData.name.length === 0 ? "XXXXXXXXXXX" : FullStudentData.name;
  const StudentHT =
    FullStudentData.hallticketNumber.length === 0
      ? "XXXXXXXXXXX"
      : FullStudentData.hallticketNumber;
  // const StudentSection =
  //   parseInt(String(StudentHT).slice(-3)) <= 60
  //     ? "A"
  //     : parseInt(String(StudentHT).slice(-3)) <= 120
  //     ? "B"
  //     : "C";
  const StudentSection2 =
    (parseInt(String(StudentHT).slice(-3)) <= 60 && "A") ||
    (parseInt(String(StudentHT).slice(-3)) <= 120 &&
      parseInt(String(StudentHT).slice(-3)) > 60 &&
      "B") ||
    (parseInt(String(StudentHT).slice(-3)) > 120 ? "C" : "XXX");

  const filteredStudents = StudentsData.slice(0, 100).filter((student) =>
    student.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
  const emptyValueForSelect =
    filteredStudents.length !== 0
      ? filteredStudents
      : [
          {
            id: "",
            hallticketNumber: "40404",
            name: "Entered Name not Available",
          },
        ];
  return (
    <div className="hall-main-container">
      <h1 className="hall-heading mb-2">Find your Hall Ticket number</h1>
      <div className="">
        <div className="search-container">
          ...
          >
            ...
            {emptyValueForSelect.slice(0, 100).map((student) => (
              <option key={student.id} value={student.hallticketNumber}>
                {student.name}
              </option>
            ))}
          </select>
        ...
        <div className="student-data-card">
          ...
          <h1>
            Roll No / Section :- {String(StudentHT).slice(-3)} /{" "}
            {StudentSection2}
          </h1>
          <h1>HT No :- {StudentHT}</h1>
        </div>
        <p className="para-of-confidetial mb-2">
          Only ICET Student details are added
          <br /> Please Enter your hall Ticket number to Display here{" "}
          <a
            href="https://forms.gle/CUyiBUVUCirisrmq7"
            target="_blank"
            rel="noreferrer"
            className="white"
          >
        ...
        <p className="para-of-confidetial">
          * If you're thinking your HT No confidential please let me know. I'll
          remove{"  "}
          <a href="whatsapp://send?phone=+918367330618" className="rise-issue">
            Rise Issue
          </a>
        </p>
      </div>
